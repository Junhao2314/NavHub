...
describe('sensitiveConfig', () => {
  ...
  describe('decryptSensitiveConfig', () => {
    ...
    it('should throw error for wrong password', async () => {
      ...
      await expect(decryptSensitiveConfig(wrongPassword, encrypted))
        .rejects.toThrow();
    });
    ...
    it('should throw error for invalid ciphertext format', async () => {
      ...
      await expect(decryptSensitiveConfig(password, 'invalid'))
        .rejects.toThrow('Invalid sensitive config payload');
      
      await expect(decryptSensitiveConfig(password, 'v2.salt.iv.data'))
        .rejects.toThrow('Invalid sensitive config payload');
    });
    ...
    it('should throw error for missing parts in ciphertext', async () => {
      ...
      await expect(decryptSensitiveConfig(password, 'v1.salt.iv'))
        .rejects.toThrow('Invalid sensitive config payload');
