...
describe('DataTab export ics', () => {
  ...
  it('excludes hidden reminders for non-admin users', async () => {
    await renderDataTab({
      syncRole: 'user',
      isPrivateUnlocked: true,
      countdownItems: [createCountdownItem({ id: 'hidden-1', hidden: true })],
    });
  ...
  it('keeps hidden reminders exportable for admins', async () => {
    const hiddenItem = createCountdownItem({ id: 'hidden-1', hidden: true });

    await renderDataTab({
      syncRole: 'admin',
      isPrivateUnlocked: false,
      countdownItems: [hiddenItem],
    });
    ...
    expect(exportService.generateIcsContent).toHaveBeenCalledWith([hiddenItem]);
    expect(exportService.downloadIcsFile).toHaveBeenCalledWith('BEGIN:VCALENDAR');
  });
});
