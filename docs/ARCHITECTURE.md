...
│       │   └── useKvSyncStrategy.ts   # 同步策略
│       ├── useAdminAccess.ts # 管理员权限
│       ├── useAppearance.ts  # 外观设置
│       ├── useLinkActions.ts # 链接操作
│       ├── usePrivacyVault.ts # 隐私分组
│       ├── useBatchEditGuards.ts # 批量编辑守卫
│       ├── useBookmarkletAddLink.ts # 书签小工具
│       ├── useDisplayedLinks.ts # 展示链接控制
│       ├── useLinkMeta.ts   # 链接元数据
│       └── ...
│
├── components/               # UI 组件
│   ├── layout/               # 布局组件
