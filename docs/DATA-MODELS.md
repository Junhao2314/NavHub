...
- `reminderMinutes` 仍按“提前 N 分钟”的数字数组持久化；订阅提醒编辑区会把它渲染为输入框下方的可删除 chip 列表。
- chip 使用完整可读文案，例如 `到点`、`一天前`、`2周前`、`1个月前`、`1年前`，而不是仅显示缩略单位。
- `reminderMinutes: []` 是合法状态，表示订阅已开启，但当前没有启用任何提醒时间。
